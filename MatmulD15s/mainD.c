...
/*    clock_t inicio,fin;
    double tTotal;
    inicio = clock();*/
    struct timespec inicioEpi,finEpi;
    double tiempoEpi;
    clock_gettime(CLOCK_MONOTONIC,&inicioEpi);
    //--------------------------------------------------------------------
    bli_sgemm_opt_mxn(k,&alpha,a1,b1,&beta,resEpiphany_,1,m,&data);       //
    //--------------------------------------------------------------------
    clock_gettime(CLOCK_MONOTONIC,&finEpi);
    tiempoEpi = (finEpi.tv_sec - inicioEpi.tv_sec);
    tiempoEpi += (finEpi.tv_nsec - inicioEpi.tv_nsec)/1000000000.0;
    printf("Tiempo Total Epi-ARM = %f -- %f GFLOPS/s\n\n",tiempoEpi,m*n*(2*k-1)/tiempoEpi/1000000000);

/*    fin = clock();
    tTotal = (double)(fin-inicio)/CLOCKS_PER_SEC;
    printf("Tiempo Total Epi-ARM = %f -- %f GFLOPS/s\n\n",tTotal,m*n*(2*k-1)/tTotal/1000000000);*/
